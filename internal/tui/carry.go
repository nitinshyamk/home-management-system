...
// exactly where a forgotten second update would show. The tree's idea of the
// carry is the third, and it goes through the same pair.
...
//
// Called from carry and drop, the two places m.copied changes, and wherever
// the surface is rebuilt -- which is the second half and the easy one to
// forget: every load makes a fresh surface, and a tree that arrived not
// knowing something was in hand would quietly start offering its items again
// -- mid-gesture, which is the only time it matters.
//
