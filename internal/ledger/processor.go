...
// applyKindChange swaps the Item's variant row along with its kind.
//
// The variant references items(id, kind), so the old row must go before the kind
// changes and the new one after. The composite foreign key from
...
func applyKindChange(ctx context.Context, q *sqlc.Queries, id domain.ItemID, ev domain.ItemKindChanged, batch []domain.Event) error {
	...
	switch ev.ToKind {
	case domain.KindUnique:
		if err := q.DeleteBulkItemVariant(ctx, int64(id)); err != nil {
			return fmt.Errorf("drop bulk variant of item %d: %w", id, err)
		}
		...
		if err := q.AddUniqueItemVariant(ctx, int64(id)); err != nil {
			return fmt.Errorf("add unique variant to item %d: %w", id, err)
		}
		return nil
